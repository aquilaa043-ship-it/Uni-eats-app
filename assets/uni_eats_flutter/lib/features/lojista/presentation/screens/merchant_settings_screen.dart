...
class _MerchantSettingsScreenState extends State<MerchantSettingsScreen> {
  ...
  // --------------------------------------------------------------------------
  // ESTADOS & CONTROLLERS (Módulo Opcional: Cardápio com Vídeos / Reels)
  // --------------------------------------------------------------------------
  final List<Map<String, dynamic>> _myProducts = [
    {
      "nome": "Costela ao Molho de Ouro",
      "preco": 79.90,
      "videoUrl": "https://storage.googleapis.com/unieats/costela_slowmo.mp4",
      "likes": 184,
    },
    {
      "nome": "Fettuccine Trufado di Parma",
      "preco": 58.50,
      "videoUrl": "https://storage.googleapis.com/unieats/fettuccine_plating.mp4",
      "likes": 95,
    }
  ];

  final TextEditingController _newProductNameController = TextEditingController();
  final TextEditingController _newProductPriceController = TextEditingController();
  final TextEditingController _newProductVideoController = TextEditingController();
  ...
  void dispose() {
    ...
    _newProductVideoController.dispose();
    super.dispose();
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // ===============================================================
              // SEÇÃO EXTRA: GERENCIADOR DE CARDÁPIO PREMIUM COM VÍDEO
              // ===============================================================
              _buildSectionCard(
                title: "Cardápio Digital com Food Reels",
                ...
                children: [
                  ...
                  const Text(
                    "Adicione vídeos de alta conversão (promocionais ou preparação de pratos) para capturar o apetite do cliente imediatamente via Food Reels.",
                    style: TextStyle(color: brandTextMuted, fontSize: 11, height: 1.4),
                  ),
                  const SizedBox(height: 16),

                  // Lista de produtos cadastrados com vídeo
                  const Text(
                    "PRATOS COM VÍDEO ATIVOS:",
                    style: TextStyle(color: brandGoldOld, fontSize: 9, fontWeight: FontWeight.bold, letterSpacing: 1),
                  ),
                  const SizedBox(height: 8),
                        ...
                        child: Row(
                          children: [
                            Container(
                              ...
                              child: const Icon(Icons.play_circle_fill, color: brandGoldOld, size: 18),
                            ),
                            const SizedBox(width: 12),
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  Text(
                                    product["nome"],
                                    style: const TextStyle(color: brandTextLight, fontSize: 13, fontWeight: FontWeight.bold),
                                  ),
                                  const SizedBox(height: 2),
                                  Text(
                                    product["videoUrl"],
                                    style: const TextStyle(color: brandTextMuted, fontSize: 10, overflow: TextOverflow.ellipsis),
                                    maxLines: 1,
                                  ),
                                ],
                              ),
                            ...
                            Text(
                              ...
                              style: const TextStyle(color: brandGoldOld, fontSize: 12, fontWeight: FontWeight.bold),
                            ),
                          ],
                        ),
                      );
                  ...
                  // Form para adicionar prato com vídeo
                  Container(
                    padding: const EdgeInsets.all(16),
                    decoration: BoxDecoration(
                      color: const Color(0xFF181818),
                      borderRadius: BorderRadius.circular(12),
                      border: Border.all(color: brandGoldOld.withOpacity(0.1), width: 0.8),
                    ),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.stretch,
                      children: [
                        const Text(
                          ...
                          style: TextStyle(color: brandTextLight, fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 1),
                        ),
                        const SizedBox(height: 12),
                        _buildPremiumTextField(
                          controller: _newProductNameController,
                          label: "Nome do Prato",
                          placeholder: "Ex: T-Bone Premium com Ervas",
                          goldColor: brandGoldOld,
                          mutedColor: brandTextMuted,
                          textColor: brandTextLight,
                        ),
                        ...
                        const SizedBox(height: 12),
                        _buildPremiumTextField(
                          controller: _newProductVideoController,
                          label: "URL do Vídeo Promocional (MP4/YouTube)",
                        ...
                        ElevatedButton.icon(
                          ...
                          onPressed: () {
                            final nome = _newProductNameController.text.trim();
                            final precoText = _newProductPriceController.text.trim();
                            final video = _newProductVideoController.text.trim();

                            if (nome.isEmpty || precoText.isEmpty || video.isEmpty) {
                              _showPremiumSnackbar("❌ Preencha todos os campos do prato.", isSuccess: false);
                              return;
                            }

                            final preco = double.tryParse(precoText.replaceAll(',', '.')) ?? 0.0;
                            if (preco <= 0) {
                              _showPremiumSnackbar("❌ Digite um preço válido maior que zero.", isSuccess: false);
                              return;
                            }
                            ...
                            setState(() {
                              ...
                              _newProductVideoController.clear();
                            });

                            _showPremiumSnackbar("✨ Novo prato de alta fidelidade cadastrado com vídeo de promoção!", isSuccess: true);
                          },
